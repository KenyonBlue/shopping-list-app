...
const AddItemModal = () => {
  ...
  const editListItem = (id) => {
    ...
    setDescription("")
    setCount("")
    setItemModal({ isModalOpen: false, item: { title: "", description: "", id: null }, status: 'add' })
  }
    ...
    <>
      {addItemModal.isModalOpen && <div className="absolute w-screen h-screen flex items-center justify-center flex-col bg-BLACK_OPACITY">
        <div className="w-full md:w-560px h-768px shadow-md bg-WHITE p-4">
          <div className="w-full p-4 font-dosis flex justify-between"> <p> SHOPPING LIST </p> <button onClick={() => setItemModal({ isModalOpen: false, item: { title: "", description: "", id: null } })}> <BiArrowToRight/> </button> </div>
          <div className="w-full px-4 font-dosis"> <p>  {addItemModal.status} an Item </p> </div>
          ...
          <Select onChange={e => setCount(e.value)} value={count ? count.value : null} options={options} />
          {addItemModal.status == "add" ? <button onClick={addItemToList} className="text-14px bg-BLUE rounded-md p-2 text-WHITE mt-6 font-nunito font-semibold">
            Add item
          </button> :
            <button onClick={() => editListItem(addItemModal?.item.id)} className="text-14px bg-BLUE rounded-md p-2 text-WHITE mt-6 font-nunito font-semibold">
              Edit item
            </button>}
        </div>
      </div>}
